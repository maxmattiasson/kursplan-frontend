...
scanOverlayImage.src = `/assets/images/bjorn-boman.jpg`
scanOverlayImage.style = `width: 4.5rem; height: 4.5rem;`
...
container.style.filter = `brightness(${ran+1})`
container.style.backgroundPosition = `center ${(Math.max((ran - 0.95), 0)/.05*200)-10}px`

scanOverlayMask.style.transform = `translateY(${shift}px)`
scanOverlayImage.style = `transform: 
translateX(0px)
translateY(${shift * -1}rem) 
skewX(${(ran - 0.5) * 20}deg);
scale: ${ran * 0.1 + 1} 1;
